...
import type { CreateMultas } from '@/types/multas.type';

interface CreateMultaModalProps {
  open: boolean;
  onClose: () => void;
}
...
export const CreateMultaModal = ({ open, onClose }: CreateMultaModalProps) => {
  ...
  const [formData, setFormData] = useState<CreateMultas>({
    nombre: '',
    codigo: '',
    departamentoId: 0,
    descripcion: '',
  ...
  const handleChange = (field: keyof CreateMultas, value: string | number) => {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle>Crear Multa</DialogTitle>
        </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Código único para identificar la multa
                    </p>
                  </div>
