...
import type { UpdateTipoPermiso, CampoPersonalizado } from '@/types/tipo-permiso.type';

interface EditTipoPermisoModalProps {
  open: boolean;
  onClose: () => void;
...
export const EditTipoPermisoModal = ({ open, onClose, tipoPermisoId }: EditTipoPermisoModalProps) => {
  ...
  // Cargar datos del tipo de permiso cuando se abra el modal
  useEffect(() => {
    if (tipoPermiso) {
      setFormData({
        nombre: tipoPermiso.nombre,
        descripcion: tipoPermiso.descripcion,
        costoBase: parseFloat(tipoPermiso.costoBase),
        numUMAsBase: parseFloat(tipoPermiso.numUMAsBase),
        numSalariosBase: parseFloat(tipoPermiso.numSalariosBase),
        vigenciaDefecto: tipoPermiso.vigenciaDefecto,
        camposPersonalizados: tipoPermiso.camposPersonalizados || { fields: [] },
      });
    }
  }, [tipoPermiso]);
  ...
  const handleChange = (field: keyof UpdateTipoPermiso, value: string | number) => {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle>Editar Tipo de Permiso</DialogTitle>
        </DialogHeader>

        {isLoadingTipoPermiso ? (
