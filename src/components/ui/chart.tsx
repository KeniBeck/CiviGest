  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          'flex aspect-video justify-center text-xs [&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-layer]:outline-none [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-sector]:outline-none [&_.recharts-surface]:outline-none',
          className
        )}
        {...props}
      >
  ...
  return (
    <style
      dangerouslySetInnerHTML={{
        __html: Object.entries({ '': 'root', '.dark': 'dark' })
          .map(([selector, theme]) =>
            colorConfig
              .map(([key, cfg]) => {
                const color = cfg.theme?.[theme] ?? cfg.color;
                return color ? `[data-chart=${id}]${selector} { --color-${key}: ${color}; }` : null;
              })
              .join('\n')
          )
          .join('\n'),
